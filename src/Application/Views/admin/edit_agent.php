...
<div class="mb-8">
    ...
    <h1 class="text-2xl font-semibold text-primary"><?= $isNew->value ? 'Nouvel Agent' : 'Modifier l\'Agent' ?></h1>
    <p class="text-sm text-secondary mt-1"><?= $isNew->value ? 'Ajoutez un nouvel agent à votre équipe.' : 'Éditez les détails de l\'agent : ' . $agent->name ?></p>
</div>

<div class="max-w-2xl">
    <form action="<?= $isNew->value ? $this->route('store-agent') : $this->route('update-agent', ['id' => $agent->id]) ?>" method="POST" class="bg-background border border-secondary/20 rounded-2xl p-6 shadow-sm space-y-6">
        ...
        <div class="pt-4">
            <button type="submit" class="w-full bg-accent hover:bg-primary text-background px-5 py-3 rounded-xl font-medium transition-colors shadow-sm flex items-center justify-center gap-2">
                <i class="fa-solid fa-check"></i> <?= $isNew->value ? 'Créer l\'agent' : 'Enregistrer les modifications' ?>
            </button>
        </div>
    </form>
</div>
